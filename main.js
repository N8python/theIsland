...
async function main() {
    ...
    const terrainMesh = new THREE.Mesh(terrainGeo, new THREE.MeshStandardMaterial({ side: THREE.DoubleSide }));
    ...
    const grassGeo = BufferGeometryUtils.mergeBufferGeometries([
        new THREE.PlaneGeometry(10, 10).applyMatrix4(new THREE.Matrix4().makeRotationY(Math.PI / 4)),
        new THREE.PlaneGeometry(10, 10).applyMatrix4(new THREE.Matrix4().makeRotationY(-Math.PI / 4)),
    ...
    instancedMesh.material.onBeforeCompile = (shader) => {
        ...
        shader.uniforms.lightProjMat = { value: directionalLight.shadow.camera.projectionMatrix };
        setTimeout(() => {
    ...
    for (let i = 0; i < 100000; i++) {
        ...
        if (Math.random() > Math.pow(normal.clone().dot(up), 10.0) ||
        ...
        let mag = 0.25;
        for (let i = 0; i < 12; i++) {
            color.add(new THREE.Vector3(noise.simplex3(noiseSamplePos.x, noiseSamplePos.y, noiseSamplePos.z), noise.simplex3(noiseSamplePos.x + 256, noiseSamplePos.y + 256, noiseSamplePos.z + 256), noise.simplex3(noiseSamplePos.x + 512, noiseSamplePos.y + 512, noiseSamplePos.z + 512)).multiplyScalar(mag));
    ...
    instancedMesh.geometry.setAttribute('grassNormal', new THREE.InstancedBufferAttribute(new Float32Array(normalAttribute), 3));
    instancedMesh.geometry.setAttribute('offset', new THREE.InstancedBufferAttribute(new Float32Array(offsetAttribute), 1));
    scene.add(instancedMesh);
    const stems = new THREE.InstancedMesh(new THREE.CylinderGeometry(0.1, 0.2, 8, 8), new THREE.MeshStandardMaterial({ color: new THREE.Color(0.5, 1, 0.0), map: new THREE.TextureLoader().load("terraincolor.png") }), 1000);
    ...
    const flowers = new THREE.InstancedMesh(new THREE.PlaneGeometry(4, 4), new THREE.MeshBasicMaterial({ side: THREE.DoubleSide, map: new THREE.TextureLoader().load("dandelion.png"), transparent: true, alphaTest: 0.125 }), 1000);
    ...
    scene.add(stems);
    scene.add(flowers);
    const bigFlower = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.2, 8, 8), new THREE.MeshStandardMaterial({ color: new THREE.Color(0.5, 1, 0.0), map: new THREE.TextureLoader().load("terraincolor.png") }));
    ...
    const bigFlowerCore = new THREE.Mesh(new THREE.SphereGeometry(0.25, 16, 16), new THREE.MeshStandardMaterial({ color: new THREE.Color(1, 1, 1), map: coreTex }));
    bigFlowerCore.position.y = 4;
    bigFlowerCore.rotation.y = -Math.PI - Math.PI / 5 + 0.1;
    bigFlower.add(bigFlowerCore);
    const spores = new THREE.InstancedMesh(new THREE.CylinderGeometry(0.005, 0.005, 1, 8), new THREE.MeshStandardMaterial({ transparent: true, opacity: 0.5 }), 1000);
    ...
    const sporeFlowers = new THREE.InstancedMesh(new THREE.PlaneGeometry(0.25, 0.25), new THREE.MeshBasicMaterial({ side: THREE.FrontSide, transparent: true, map: new THREE.TextureLoader().load("dandelionspore.png"), alphaTest: 0.125 }), 1000);
    const spores2 = new THREE.InstancedMesh(new THREE.CylinderGeometry(0.005, 0.005, 1, 8), new THREE.MeshStandardMaterial({ transparent: true, opacity: 0.5 }), 1000);
    ...
    const sporeFlowers2 = new THREE.InstancedMesh(new THREE.PlaneGeometry(0.25, 0.25), new THREE.MeshBasicMaterial({ side: THREE.FrontSide, transparent: true, map: new THREE.TextureLoader().load("dandelionspore.png"), alphaTest: 0.125 }), 1000);
    scene.add(spores2);
    scene.add(sporeFlowers2);
    //sporeFlowers.geometry.applyMatrix4(new THREE.Matrix4().makeRotationX(Math.PI / 2));
    sporeFlowers.geometry.applyMatrix4(new THREE.Matrix4().makeTranslation(0, 0.0, 1.0));
    sporeFlowers2.geometry.applyMatrix4(new THREE.Matrix4().makeTranslation(0, 0.0, 1.0));
    dummy = new THREE.Object3D();
    ...
    const reflectTerrain = new THREE.Mesh(terrainMesh.geometry, terrainMesh.material);
    ...
    const reflectTexture = new THREE.WebGLRenderTarget(clientWidth, clientHeight, {
        minFilter: THREE.LinearFilter,
        magFilter: THREE.NearestFilter
    });
    reflectTexture.depthTexture = new THREE.DepthTexture(clientWidth, clientHeight, THREE.FloatType);
    // Post Effects
    ...
    const makeTree = (geometries, position, bottomSize, topSize, direction, height, tier = 0) => {
        //console.log(position);
        const cylinderGeo = new THREE.CylinderGeometry(topSize, bottomSize, height, Math.max(32 / Math.pow(2, tier), 4));
        cylinderGeo.applyMatrix4(new THREE.Matrix4().makeTranslation(0, height / 2, 0));
        cylinderGeo.applyMatrix4(new THREE.Matrix4().makeRotationX(Math.PI / 2));
        cylinderGeo.applyMatrix4(makeTBN(direction));
        cylinderGeo.applyMatrix4(new THREE.Matrix4().makeTranslation(position.x, position.y, position.z));
        cylinderGeo.setAttribute('leafiness', new THREE.BufferAttribute(new Float32Array(Array(cylinderGeo.attributes.position.count).fill(Math.max(tier - 1, 0))), 1));
        ...
        if (topSize > 0.1) {
            ...
            for (let i = 0; i < branchAmount; i++) {
                let dir;
                for (let i = 0; i < 100; i++) {
                    dir = direction.clone().add(new THREE.Vector3(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5)).normalize();
                    let moveAlong = false;
            ...
            const joint = new THREE.SphereGeometry(topSize, Math.max(32 / Math.pow(2, tier), 4), Math.max(32 / Math.pow(2, tier), 4));
            joint.setAttribute('leafiness', new THREE.BufferAttribute(new Float32Array(Array(joint.attributes.position.count).fill(0)), 1));
            const newPosition = position.clone().add(direction.clone().multiplyScalar(height));
            joint.applyMatrix4(new THREE.Matrix4().makeTranslation(newPosition.x, newPosition.y, newPosition.z));
            geometries.push(joint);
            for (let i = 0; i < directions.length; i++) {
                makeTree(geometries, newPosition, topSize, topSize / (1.875 + Math.random() * 0.25), directions[i], height * (0.625 + 0.25 * Math.random()), tier + 1);
            }
    ...
    for (let i = 0; i < 100; i++) {
        ...
        const treeMesh = new THREE.Mesh(BufferGeometryUtils.mergeBufferGeometries(treeParts), new THREE.MeshStandardMaterial({ color: new THREE.Color((144 / 255) * (0.75 + 0.5 * Math.random()), (92 / 255) * (0.75 + 0.5 * Math.random()), (66 / 255) * (0.75 + 0.5 * Math.random())), normalMap: barkMap, map: barkColor }));
        ...
        while (treeMesh.position.y < 10 || surfaceDir.dot(targetDir) < 0.95 ||
            treePositions.some(position => Math.hypot(position.x - treeMesh.position.x, position.z - treeMesh.position.z) < 10)) {
...
void main() {
	...
	vWorldPosition = worldPosition.xyz;
        ...
        const sampler = new MeshSurfaceSampler(treeMesh).setWeightAttribute('leafiness').build();
    ...
    for (let i = 0; i < 100; i++) {
        ...
        let surfaceDir = new THREE.Vector3();
        let targetDir = new THREE.Vector3(0, -1, 0);
        while (lodMesh.position.y < 10 || surfaceDir.dot(targetDir) < 0.95 ||
            treePositions.some(position => Math.hypot(position.x - lodMesh.position.x, position.z - lodMesh.position.z) < 10) ||
            rockPositions.some(position => Math.hypot(position.x - lodMesh.position.x, position.z - lodMesh.position.z) < 10)) {
        ...
        lodMesh.scale.set(0.5 + 1 * Math.random() + 0.5 * verticalMajor, verticalMajor ? 0.5 + 1.25 * Math.random() : 0.5 + 0.5 * Math.random(), 0.5 + 1 * Math.random() + 0.5 * verticalMajor);
        const rockIndex = Math.floor(Math.random() * rockTextures.length);
        const rockMat = new THREE.MeshStandardMaterial({ color: new THREE.Color(0.75 + Math.random() * 0.25, 0.75 + Math.random() * 0.25, 0.75 + Math.random() * 0.25), map: rockTextures[rockIndex], normalMap: rockNormals[rockIndex] });
        ...
        for (let level = 0; level < levelGeos.length; level++) {
            ...
            for (let j = 0; j < posAttr.count; j++) {
                ...
                for (let k = 0; k < 12 - 2 * level; k++) {
                    amt += mag * noise.simplex3(freq * position.x + i * 100 + 512 + shift, freq * position.y + i * 200 + 1024 + shift, freq * position.z + i * 300 + 2048 + shift);
                    mag /= 2;
                    freq *= 2;
                    shift += 128.0;
                }
        ...
        lodMesh.lookAt(lodMesh.position.clone().add(surfaceDir.clone().multiplyScalar(5)));
    ...
    for (let i = 0; i < 1000; i++) {
        ...
        if (Math.random() > Math.pow(normal.clone().dot(up), 10.0) ||
            Math.random() > dummy.position.y * 0.1 ||
            treePositions.some(position => Math.hypot(position.x - dummy.position.x, position.z - dummy.position.z) < 10) ||
            rockPositions.some(position => Math.hypot(position.x - dummy.position.x, position.z - dummy.position.z) < 10)) {
            i--;
            continue;
        }
        let target = dummy.position.clone().add(normal);
        ...
        flowerBoxes.push(new THREE.Box3().setFromCenterAndSize(dummy.position.clone().add(normal.clone().multiplyScalar(-5)), new THREE.Vector3(5, 10, 5)));
    }
    ...
    document.addEventListener("mousedown", e => {
        ...
        if (controls.isLocked && !bigFlower.visible) {
            ...
            flowerBoxes.sort((a, b) => {
                return a.getCenter(new THREE.Vector3()).distanceTo(camera.position) -
    ...
    function animate() {
        ...
        if (mouseDown && bigFlower.visible) {
            for (let i = 0; i < 10; i++) {
                ...
                spores.instanceMatrix.needsUpdate = true;
                sporeFlowers.instanceMatrix.needsUpdate = true;
                removedVelocities[removeIndex] = camera.getWorldDirection(new THREE.Vector3()).add(new THREE.Vector3(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5).multiplyScalar(0.25)).normalize().add(playerCapsule.horizontalVelocity.clone().multiplyScalar(25));
                removedAccelerations[removeIndex] = new THREE.Vector3(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5).normalize();
                removeIndex++;
            }
        }
        if (removeIndex > 999) {
        ...
        for (let i = 0; i < 1000; i++) {
            ...
            m.premultiply(new THREE.Matrix4().makeTranslation(removedVelocities[i].x * 0.25, removedVelocities[i].y * 0.25, removedVelocities[i].z * 0.25));
            ...
            removedVelocities[i].add(removedAccelerations[i].clone().multiplyScalar(0.01));
        ...
        camera.position.y += slowOffset + (fastOffset - slowOffset) * Math.min(10 * playerCapsule.horizontalVelocity.length(), 1);
        const speedFactor = 0.33;
        if (controls.isLocked) {
            if (keys["w"]) {
                playerCapsule.horizontalVelocity.add(getForwardVector(camera).multiplyScalar(speedFactor * delta));
            }
            ...
            if (keys["a"]) {
                playerCapsule.horizontalVelocity.add(getSideVector(camera).multiplyScalar(-speedFactor * delta));
            }
        ...
        ssaoPass.uniforms["projViewMat"].value = camera.projectionMatrix.clone().multiply(camera.matrixWorldInverse.clone());
        ssaoPass.uniforms["projectionMatrixInv"].value = camera.projectionMatrixInverse;
        ssaoPass.uniforms["viewMatrixInv"].value = camera.matrixWorld;
        ssaoPass.uniforms["cameraPos"].value = camera.position;
        ssaoPass.uniforms['resolution'].value = new THREE.Vector2(clientWidth, clientHeight);
        ...
        godRayPass.uniforms["projViewMat"].value = camera.projectionMatrix.clone().multiply(camera.matrixWorldInverse.clone());
        godRayPass.uniforms["projectionMatrixInv"].value = camera.projectionMatrixInverse;
        godRayPass.uniforms["viewMatrixInv"].value = camera.matrixWorld;
        godRayPass.uniforms["cameraPos"].value = camera.position;
        godRayPass.uniforms['resolution'].value = new THREE.Vector2(clientWidth, clientHeight);
